...
if ( ! function_exists( 'ten_blog_setup' ) ) :
	/**
	 ...
	 */
	function ten_blog_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Ten Blog, use a find and replace
		 * to change 'ten-blog' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus( array(
			'primary_menu' 		=> esc_html__( 'Primary Menu', 'ten-blog' ),
		) );
...
add_action( 'after_setup_theme', 'ten_blog_setup' );
...
* Enqueue theme fonts.
*/
function ten_blog_fonts() {
	$fonts_url = ten_blog_get_fonts_url();

	// Load Fonts if necessary.
	if ( $fonts_url ) {
		require_once get_theme_file_path( 'inc/wptt-webfont-loader.php' );
		wp_enqueue_style( 'ten-blog-fonts', wptt_get_webfont_url( $fonts_url ), array(), null );
	}
}
add_action( 'wp_enqueue_scripts', 'ten_blog_fonts', 1 );
add_action( 'enqueue_block_editor_assets', 'ten_blog_fonts', 1 );
